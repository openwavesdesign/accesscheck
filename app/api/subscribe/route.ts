...
export async function POST(req: Request) {
  let body: { email?: string; url?: string; grade?: string; checks?: unknown; score?: number };
  try {
    body = await req.json();
  } catch {
  ...
  const { email, url, grade, checks, score } = body;
  ...
  const token = signToken({ email: normalizedEmail, code, expiresAt, url: url ?? '', grade: grade ?? '', checks: checks ?? [], score: score ?? 0 });
